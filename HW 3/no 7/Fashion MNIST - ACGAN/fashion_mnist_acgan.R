...
# Loade fashion mnist data, and force it to be of shape (..., 1, 28, 28) with
# range [-1, 1]
...
fashion_mnist$train$x <- (fashion_mnist$train$x - 127.5)/127.5
fashion_mnist$test$x <- (fashion_mnist$test$x - 127.5)/127.5
fashion_mnist$train$x <- array_reshape(fashion_mnist$train$x, c(60000, 1, 28, 28))
fashion_mnist$test$x <- array_reshape(fashion_mnist$test$x, c(10000, 1, 28, 28))

num_train <- dim(fashion_mnist$train$x)[1]
num_test <- dim(fashion_mnist$test$x)[1]
...
for(epoch in 1:epochs){
  ...
  for(index in 1:num_batches){
    ...
    image_batch <- fashion_mnist$train$x[batch,,,,drop = FALSE]
    label_batch <- fashion_mnist$train$y[batch]
...
generator %>% save_model_hdf5("generator_acgan_fashion_mnist.h5")

#for load the saved model 
